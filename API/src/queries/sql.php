...
function buscarMateria(mysqli $conn, $nombreMateria) {
    // Filtrar y escapar la variable de entrada
    $nombreMateria = $conn->real_escape_string($nombreMateria);

    // Consulta SQL para buscar una materia por nombre
    $query = "SELECT * FROM materias WHERE nombre = '$nombreMateria' LIMIT 1";

    // Ejecutar la consulta
    $result = $conn->query($query);

    if ($result->num_rows > 0) {
        // La materia existe en la base de datos
        $row = $result->fetch_assoc();
        
        // Obtener el ID y otros datos relacionados con la materia
        $idMateria = $row['id'];
        $otroDatoMateria = $row['otro_dato'];  // Ajusta el nombre de la columna según tu estructura

        // Puedes agregar más datos según sea necesario

        $datosMateria = array(
            'id' => $idMateria,
            'otro_dato' => $otroDatoMateria
            // Agrega más datos si es necesario
        );

        return $datosMateria;
    } else {
        // La materia no existe en la base de datos
        return null;
    }
}
